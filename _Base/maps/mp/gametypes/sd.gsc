...
onEndUse( team, player, result )
{
	...
	player maps\mp\gametypes\_hud_hints::hideHint( "quick_defuse" );
...
onDrop( player )
{
	if ( !level.bombPlanted )
	{
		...
		{
			player maps\mp\gametypes\_hud_hints::hideHint( "bomb_stolen" );
		}
...
onPickup( player )
{
	...
	if ( isDefined( player ) && player.pers["team"] == game["defenders"] ) {
		...
		{
			player maps\mp\gametypes\_hud_hints::showHint( &"OW_BOMB_STOLEN", "bomb_stolen" );
		}
		else
		{
			player maps\mp\gametypes\_hud_hints::showHint( &"OW_BOMB_DROP", "bomb_stolen" );
		}
	}
